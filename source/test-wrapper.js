...
try {
  const file = yaml.safeLoad(fs.readFileSync("../examples/name-alert.yml", "utf8"))
    ...
    for (const option of file.options) {
      ...
      switch (name) {
        case "alerts":
          ...
          else await t.setNativeDialogHandler(() => false)
          break
      }
    }
    ...
    for (const item in file.steps) {
      ...
      switch (command) {
        case "write":
          ...
          await t
            .typeText(writeElement, writeValue)
            .takeScreenshot(
              `${item}-write-${writeValue}-in-${writeElementName}`
            )
          break
        ...
        case "select":
          ...
          await t
            ...
            .takeScreenshot(
              `${item}-select-${selectValue}-in-${selectElementName}`
            )
          break
        ...
        case "click in":
          ...
          await t
            .click(clickElement)
            .takeScreenshot(`${item}-click-in-${clickElementName}`)
          break
        ...
        case "assert if":
          ...
          const assertAttribute = assertDetails
            .dropRight()
            .dropRight()
            .last()
          ...
          if (assertType.toLowerCase() == "equals") {
            await t
              .expect(Selector(element)[assertAttribute])
              .eql(_instructions.dropRight().last())
          }
          break
      }
    }
